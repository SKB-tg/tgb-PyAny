...
//-------
    ...
    delete tg_data.str;
    tg_data['_auth']=js initData;
    let tg_data_json = JSON.stringify(tg_data);
    //console.log(tg_data);
//-------------
    ...
    xhr.open('POST', url);
    xhr.setRequestHeader('Content-Type', 'application/join; charset=utf-8');//'application/x-www-form-urlencoded');
    ...
    xhr.send(tg_data_json);
    //setTimeout(sendMessage(tg_data), 1000);//вызов функции отсылки в чат, можно переделать забор из формы 
  };
//----------------------
...
function chek_order(resp) {
  ...
  // const renderCartItem = ({ id, name, attribute, src, price, quantity }) => {
    ...
    const cartItemTemplate = `
      <p class="popup__subtitle">Чек-ордер №25-д</p>
      <p>${resp[0]}</p>
      <p>${resp[1]}</p>
      <p>${resp[2]}</p>
      <p>${resp[3]}</p>
      <p>${resp[resp.length - 4]}</p>
      <p>${resp[resp.length - 3]}</p>
      <p>${resp[resp.length - 2]}</p>
      <p>${resp[resp.length - 1]}</p>
      `;
